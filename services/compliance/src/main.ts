...
async function bootstrap() {
  ...
  app.useGlobalGuards(new GuardaSimulador(), new GuardaEscopo(app.get(Reflector), 'services/compliance'));

  const porta = Number(process.env.PORTA_COMPLIANCE ?? 3001);
  await app.listen(porta, '0.0.0.0');
  process.stdout.write(JSON.stringify({ servico: 'compliance', porta, ambiente: ambiente(), ok: true }) + '\n');
}
bootstrap();
