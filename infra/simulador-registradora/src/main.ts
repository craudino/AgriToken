...
async function bootstrap() {
  ...
  app.useGlobalGuards(new GuardaSimulador(), new GuardaEscopo(app.get(Reflector), 'infra/simulador-registradora'));

  const porta = Number(process.env.PORTA_REGISTRADORA ?? 3005);
  await app.listen(porta, '0.0.0.0');
  process.stdout.write(JSON.stringify({ servico: 'simulador-registradora', porta, ambiente: ambiente(), ok: true }) + '\n');
}
bootstrap();
