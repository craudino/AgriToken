...
async function bootstrap() {
  ...
  app.useGlobalGuards(new GuardaSimulador(), new GuardaEscopo(app.get(Reflector), 'services/eudr'));

  const porta = Number(process.env.PORTA_EUDR ?? 3004);
  await app.listen(porta, '0.0.0.0');
  process.stdout.write(JSON.stringify({ servico: 'eudr', porta, ambiente: ambiente(), ok: true }) + '\n');
}
bootstrap();
