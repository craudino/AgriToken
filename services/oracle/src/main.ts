...
async function bootstrap() {
  ...
  app.useGlobalGuards(new GuardaSimulador(), new GuardaEscopo(app.get(Reflector), 'services/oracle'));

  const porta = Number(process.env.PORTA_ORACLE ?? 3002);
  await app.listen(porta, '0.0.0.0');
  process.stdout.write(JSON.stringify({ servico: 'oracle', porta, ambiente: ambiente(), ok: true }) + '\n');
}
bootstrap();
